import type { ModelEntry, TaskType } from "@tokenix/types"
...
export function getBaseline(): ModelEntry {
...
export function patchAcpi(updates: Array<{ model_id: string; acpi: number }>) {
  for (const { model_id, acpi } of updates) {
    const entry = ALL_MODELS.find((m) => m.model_id === model_id)
    if (entry) entry.acpi = acpi
  }
}
